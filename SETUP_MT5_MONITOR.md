...
下载整个仓库后(见下面第三步),先做好准备:装好 MT5、登录、把黄金 EA 挂上。然后:

1. 装 Python(见第二步,只需一次,记得勾 "Add Python to PATH")
2. **双击 `install_mt5_monitor.bat`** —— 自动装好需要的库
3. **双击 `run_monitor.bat`** —— 试跑一次,看结论
4. **右键 `schedule_monitor.bat` → 以管理员身份运行** —— 设成每 30 分钟自动跑
5. (可选)想让提醒推到手机:用记事本打开 `monitor_config.bat` 按里面说明填 Telegram

就这样。下面是每一步的详细说明,遇到问题再看。
