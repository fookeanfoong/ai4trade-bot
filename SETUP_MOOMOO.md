...
export MOOMOO_BOOK_EQUITY=200           # 逻辑账本,按 $200 计划下单(不按模拟盘的虚拟余额)
export MOOMOO_RISK_PCT=0.05             # 每笔止损最多亏账本 5%
export MOOMOO_LEVERAGE_CAP=2.0          # 真实美股保证金约 2x(不是模拟盘的 10x)
```
...
```cron
*/10 13-20 * * 1-5   cd /path/to/ai4trade-bot && git pull -q && MOOMOO_TRD_ENV=SIMULATE python moomoo_paper_trader.py >> moomoo.log 2>&1
```
