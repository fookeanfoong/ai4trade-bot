...
# 盈亏比下限,**默认关闭(0)**。
#
# 这道门槛一直在拿错误的数字做判断:这里的 stop_pct 是「支撑位下方」的止损,
# 而引擎收到信号后会按 MIN_RR_NET 把止损收紧到「目标距离」——那个支撑位止损
# 根本不会被采用。于是信号层算出 rr=0.88 就拒绝,而这笔交易实际盈亏比是 2.2。
#
# 实盘快照里 ADA(0.98)和 LINK(0.88)正是这样被挡在门外:两个都只差一点点,
# 差的还是一个假数字。真正的风险回报由引擎结构性保证(止损距离 <= 目标距离,
# 至少 1:1),这里再筛一遍只会误杀。
#
# 之前把它调到 1.5 时回测显示上涨行情一笔都开不出来,当时只把阈值降回 1.0,
# 治了症状没治病根。现在默认关掉;「空间够不够」交给 MIN_TARGET_ROOM 判断。
MIN_RR = float(os.environ.get("MIN_RR", "0.0"))
