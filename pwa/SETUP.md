...
> ⚠️ **别让机器人的提交触发自动部署**。交易机器人每 15 分钟就往 `main` 推一次状态
> （一天约 90 次）。若每次推送都构建，会撞爆 Vercel 免费版「每天 100 次部署」的上限，
> 之后部署全部失败、邮箱被失败通知刷屏。所以 `pwa/vercel.json` 里写死了
> `"ignoreCommand": "exit 0"` —— **Vercel 的退出码是反的：`exit 0` = 跳过构建，
> `exit 1` = 照常构建**。想恢复自动部署再改回 `exit 1`，别改反。
> 需要发布新版本时，走 `deploy-site` 工作流的 Deploy Hook 手动触发。
