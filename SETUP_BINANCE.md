...
## 补充:换币种 / 快进快出(在 `binance_keys.bat` 里加一行,不用重下)

机器人从 `binance_keys.bat` 读这些开关,加好后双击 `run_binance.bat` 就生效:

- **改成做 BTC**(BTC 同时还是大盘风险参考):
  ```
  set CRYPTO_SYMBOLS=BTC
  ```
  想同时盯 BTC 和 ETH、哪个有机会做哪个:`set CRYPTO_SYMBOLS=BTC,ETH`(默认)。
- **快进快出(快速模式)**:
  ```
  set SCALP_MODE=fast
  ```
  它会切成:**5 分钟 K 线、每 5 分钟一轮、最小持仓 5 分钟、到点就走不恋战**。

两个能一起用。例:只做 BTC + 快进快出 —— 在 `binance_keys.bat` 里加这两行:
```
set CRYPTO_SYMBOLS=BTC
set SCALP_MODE=fast
```

> ⚠️ 实话:**越快 ≠ 越赚**。这套的回测里更短的周期大多是噪音 + 手续费,反而比 1 小时**亏得多**。测试网上随便试,但别指望"快"能变出收益;真钱阶段我更建议默认的 1h。
